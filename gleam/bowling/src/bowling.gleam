...
pub type Frame {
  Frame(rolls: List(Int), bonus: List(Int), index: Int)
}
...
pub fn roll(game: Game, knocked_pins: Int) -> Result(Game, Error) {
  ...
  let standing_pins_last_frame = case most_recent_frame.index, most_recent_frame.rolls {
    ...
    10, [10, a] if a + knocked_pins > 10 -> Error(InvalidPinCount)
    //10, []
  ...
  {
    knocked_pins, False, Ok(standing_pins_last_frame) if knocked_pins > standing_pins_last_frame -> Error(InvalidPinCount)

    knocked_pins, False, _ -> Ok(Frame(..most_recent_frame, rolls: [knocked_pins, ..most_recent_frame.rolls]))
    // update the open frame
    knocked_pins, True, _ -> Ok(Frame(rolls: [knocked_pins], bonus: [], index: most_recent_frame.index + 1))
...
pub fn score(_game: Game) -> Result(Int, Error) {
  Ok(300)
}

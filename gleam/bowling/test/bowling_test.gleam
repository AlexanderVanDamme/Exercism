import bowling.{
  type Error, Game, Frame, GameComplete, GameNotComplete, InvalidPinCount, roll, score, is_frame_complete
}
...
pub fn last_roll_in_tenth_frame_test () {
  let game = Game([Frame([10,5],10)])
  roll(game,6)
  |> should.equal(Error(InvalidPinCount))
}

pub fn is_frame_complete_test () {
  let frame = Frame([0],2)
  is_frame_complete(frame)
  |> should.equal(False)
}
